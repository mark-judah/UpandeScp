  ...
  const load = () => {
    ...
    fetchDraftTransfers({
      ...
      .then((r) => {
        ...
        setSelected((prev) => {
          // Selections that no longer exist, and any that have become
          // unsendable since. Both read off the rows that just arrived, so a
          // draft cannot stay ticked for a problem it no longer has, and cannot
          // stay stuck once the batch is fixed: the next load says
