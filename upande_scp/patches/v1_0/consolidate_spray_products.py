...
def _drop_dead_doctypes():
	...
	_drop_orphaned_foliar_table()


def _drop_orphaned_foliar_table():
	"""Drop `tabFoliar` when its DocType record has already gone.

	`bench migrate` removes orphan doctypes — those whose folder is no longer on
	disk — *before* post_model_sync patches run. Deleting the Foliar folder as
	part of this consolidation therefore means the DocType record is gone by the
	time the patch above looks for it, so the delete is skipped and the physical
	table is left behind holding a stale copy of every foliar. Harmless to read,
	because nothing references it any more, but it is duplicated data sitting
	next to the real thing, and the next person to debug a foliar will find it.

	Guarded, not blind: the table is dropped only once every row in it is
	confirmed present in Spray Product. If absorption skipped any — a foliar
	whose Item no longer exists, which is what `_absorb_foliars` reports — the
	table stays and says why, because dropping it would destroy the only
	remaining record of what was missed.
	"""
	if frappe.db.exists("DocType", FOLIAR) or not frappe.db.table_exists(FOLIAR):
		return

	orphans = frappe.db.sql(
		f"""SELECT f.name FROM `tab{FOLIAR}` f
		     LEFT JOIN `tab{NEW}` p ON p.name = f.name
		    WHERE p.name IS NULL""",
		pluck=True,
	)
	if orphans:
		frappe.log_error(
			title="consolidate_spray_products: tabFoliar not dropped",
			message=(
				f"{len(orphans)} row(s) in tabFoliar have no Spray Product: "
				f"{', '.join(orphans[:20])}. The table was left in place."
			),
		)
		return

	frappe.db.sql(f"DROP TABLE `tab{FOLIAR}`")
	frappe.logger().info("consolidate_spray_products: dropped orphaned tabFoliar")
