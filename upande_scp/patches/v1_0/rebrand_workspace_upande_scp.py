...
The workspace itself ships as a module workspace (upande_scp/workspace/upande_scp),
but the desk tile name, its icon and the sidebar link targets live on
auto-generated ``Workspace Sidebar`` and ``Desktop Icon`` records which are NOT
part of that folder — so re-apply them here. Idempotent: safe to re-run and a
no-op once applied.
...
def execute():
    ...
    #    The NEW workspace already exists at this point (synced from the module
    #    workspace in sync_all, which runs before post_model_sync patches), so
    #    repointing to it is valid.
    ...
    # 1. Workspace — the module workspace ships "Upande SCP"; rename/clean any
    #    lingering old doc so a site doesn't end up with both.
