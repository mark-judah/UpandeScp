...
def getTrapData(week):
    ...
    try:
        ...
        end_date = date.fromisocalendar(int(year_str), int(week_str), 7)
        ...
        scouting_entries = frappe.get_all(
            ...
            fields=[
                'name', 'greenhouse', 'bed', 'zone', 'date_of_capture',
                'time_of_capture', 'latitude', 'longitude',
            ],
            limit_page_length=0,
        )
        ...
        return {
            ...
            'greenhouses': sorted(greenhouses_set),
            'pests': sorted(pests_set),
            'week': week,
