...
def _is_duplicate(employee_name, greenhouse, date_of_capture, time_of_capture, bed, zone):
    """
    Returns True if an entry exists within a 3-second window of time_of_capture
    for the same scout, greenhouse, and date.
    """
    try:
        time_obj = datetime.strptime(time_of_capture, "%H:%M:%S")
    except (ValueError, TypeError):
        # If time can't be parsed fall back to exact match
        filters = {
    ...
    try:
        ...
        for entry_data in data_list:
            try:
                ...
                # Normalize zone_message to a dict — the function can return a plain
                # string when no zone is found, which would crash the metadata insert.
                if not isinstance(zone_message, dict):
                    zone_message = {"distance": "0.0", "buffer": "0.0", "fallback": False}

                # Zone is required when a bed is provided. geo_utils already falls back
                # to all zones, so None here means no zone geometry exists at all.
                ...
                # --- Employee lookup (must be before duplicate check) ---
