...
def submit_without_biometric(names: str | list) -> dict:
    ...
    for name in names:
        try:
            ...
            # Same as the biometric path: the rule picks the batches nobody
            # chose, and only an expired one or an empty store stops this row.
            filled = autofill_batches(doc)
...
# ----------------------------------------------------------------------
# Batches on a draft transfer
# ----------------------------------------------------------------------
# Batch tracking is on for 533 chemical items, so a transfer cannot be
# submitted until every outgoing row names a batch — and nothing in SCP
# ever set one. These three read availability, propose a batch per row,
# and write back what the storesman accepted. The rule itself lives in
# `batch_suggestion`, kept pure and tested away from the database.
...
def apply_transfer_batches(name: str, picks) -> dict:
    ...
    for idx, batch_no in picks.items():
        ...
        if current and current != batch_no:
            ...
            # the page that has to change before the transfer can go at all, and
            # making the storesman clear it in the desk first is the step that
            # does not happen.
            expiry = frappe.db.get_value("Batch", current, "expiry_date")
