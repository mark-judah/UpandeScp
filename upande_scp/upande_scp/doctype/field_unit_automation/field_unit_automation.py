...
So there is one tool, and `unit_type` says which vocabulary is in play:

| Unit Type | Used by | Children created |
|-----------|---------|------------------|
| `Bed`     | roses   | `Zone`           |
| `Row`     | avocado | `Orchard Tree`   |
| `Band`    | coffee  | `Orchard Tree`   |

`Band` behaves exactly like `Row` because it *is* a row. Nothing downstream has
to learn a new structure; a coffee band is a `Bed` with `unit_type = "Band"`, and
the scouting screens, maps and mobile bundle read it the way they already read
the other two.
...
# unit_type → the child doctype it holds.
_CHILD_SPEC = {
	"Bed": "Zone",
	"Row": "Orchard Tree",
	"Band": "Orchard Tree",
...
# Which field on the child links back to its unit (a `Bed` row either way).
_CHILD_PARENT_FIELD = {
	"Zone": "bed",
	"Orchard Tree": "row",
}

# The child's own number. Both are Int and mandatory, and both doctypes name
# themselves off it, so this is also what makes a re-run idempotent.
_CHILD_NUMBER_FIELD = {
	"Zone": "zone",
	"Orchard Tree": "tree",
}

# `Orchard Tree` carries the number twice: `tree` (Int) is mandatory and is what
# the document names itself from, while `tree_number` (Data) is what every reader
# in this app actually queries — `get_model_trees`, `get_orchard_trees`, the
# scouting payloads — and what all 53,699 existing trees are populated with. The
# superseded automation wrote only `tree_number`, leaving the mandatory field
# empty, which is why no tree could be created at all once core took ownership of
# this doctype. New trees set both.
_CHILD_MIRROR_FIELD = {
	"Orchard Tree": "tree_number",
}
...
class FieldUnitAutomation(Document):
	...
	def run_automation(self):
		...
		if unit_type not in _CHILD_SPEC:
		...
		child_doctype = _CHILD_SPEC[unit_type]
		parent_field = _CHILD_PARENT_FIELD[child_doctype]
		number_field = _CHILD_NUMBER_FIELD[child_doctype]
		...
		unit_label = unit_type.lower()
		child_label = child_doctype.lower() if child_doctype == "Zone" else "tree"
		summary = (
			f"{units_created} {unit_label}s created, {units_skipped} existed. "
			f"{children_created} {child_label}s created, {children_skipped} existed."
		)
		if unparsable:
	...
	def _get_or_create_child(
		...
		existing = frappe.db.exists(
			{
				...
				parent_field: unit.name,
				number_field: child_number,
			}
		)
		if existing:
			return False
		...
		mirror = _CHILD_MIRROR_FIELD.get(child_doctype)
		if mirror:
			payload[mirror] = child_number
		# Zones carry the greenhouse; Orchard Trees carry the block. Same column
		# in `tabBed`, different field name on the child.
		if child_doctype == "Zone":
			payload["greenhouse"] = warehouse
		else:
			payload["block"] = warehouse
...
def run_from_file(warehouse, geojson_path, unit_type="Bed", sectors=None):
